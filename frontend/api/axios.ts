...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    const isAuthEndpoint = originalRequest.url?.includes('/api/auth/google') || 
    ...
    if ((error.response?.status === 401 || error.response?.status === 403) && 
      ...
      try {
        ...
        if (!newAccessToken) {
          await logout("/login");
          throw new Error("새로운 access token 발급 실패");
        }
        ...
        dev.log("원래 요청 재시도:", {
          ...
          authHeaderValue: originalRequest.headers.Authorization?.substring(0, 20) + "..."
        });
